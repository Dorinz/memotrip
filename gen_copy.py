...
def digest(spec: dict) -> str:
    ...
    for i, it in enumerate(spec.get("timeline", [])):
        ...
        if t == "transit":
            out.append(
                f"{i}. TRANSIT {it.get('mode', '')} {it.get('date', '')}: "
                f"{it.get('from', '')} -> {it.get('to', '')}  {it.get('route', '')}".rstrip()
            )
        elif t == "drama":
            frm, to = it.get("from", ""), it.get("to", "")
            out.append(f"{i}. DRAMA (flight disruption): {frm} <-> {to}")
        elif t == "day":
            out.append(
                f"{i}. DAY key={it.get('key', '')} day={it.get('dayIndex')}/{it.get('dayCount')} "
...
def _client(model: str):
    ...
    if not api_key:
        sys.exit(
            "gen_copy needs GEMINI_API_KEY in the environment (prose can't be generated offline)."
        )
    import logging
        ...
        "hero": {
            ...
            "properties": {
                ...
                "meta": {
                    ...
                    "items": {
                        "type": "object",
                        "properties": {"n": {"type": "string"}, "l": {"type": "string"}},
                        "required": ["n", "l"],
                    },
                },
            },
        ...
        "days": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "title": {
                        "type": "string"
                    },  # check-in: small line above "City, Island"; else: the big heading
                    "island": {"type": "string"},  # fill if missing (check-in days only)
                    "lede": {"type": "string"},  # 1-2 sentences on THAT day
                    "about": {
                        "type": "string"
                    },  # check-in days only: one fact about the place + why
                    "tags": {
                        "type": "array",
                        "items": {"type": "string"},
                    },  # 2-4 short activity chips for that day
                },
...
DRAMA_LINE = (
    "- drama: the itinerary has a flight disruption. eyebrow (date + what "
...
def generate_copy(
    ...
    *,
    ...
    days_blob = "\n".join(
        ...
        f"{d.get('city', '')}, {d.get('island', '') or '?'}  "
        f"{d.get('date') or (d.get('dateRange') or [''])[0]}"
    ...
    days_prompt = DAYS_PROMPT.format(
        lang=lang,
        description=description.strip(),
        docs=(docs_text[:20000] or "(none provided)"),
        days=days_blob,
    )
    ...
    try:
        ...
        for block in ("hero", "outro"):
            ...
            fill(
                spec[block],
                trip.get(block, {}),
                ("eyebrow", "h1", "h2", "sub", "p", "region", "meta", "stats"),
                overwrite,
            )
...
def main() -> int:
    ...
    ap.add_argument(
        "--docs", nargs="*", default=[], help="optional logistics globs for activity detail"
    )
    ap.add_argument("--out", default="trip_spec.copy.json")
    ap.add_argument(
        "--in-place", dest="in_place", action="store_true", help="write back into --spec"
    )
    ap.add_argument(
        "--overwrite", action="store_true", help="regenerate fields that already have text"
    )
    ap.add_argument("--model", default="gemini-3.6-flash")
    ap.add_argument("--lang", default="he")
    ap.add_argument(
        "--dry-run", action="store_true", help="assemble prompts, don't call the API or write"
    )
    a = ap.parse_args()
    ...
    if not a.in_place:
        print(
            "  review it, then copy the good parts into trip_spec.json (or re-run with --in-place)"
        )
